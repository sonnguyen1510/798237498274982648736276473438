...
public class Learning_SA_photo_question extends AppCompatActivity {
    private List<SA_photo_question> data;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_learning_sa_photo_question);

        Previous = findViewById(R.id.Learning_sa_photos_previous_question);
        Next = findViewById(R.id.Learning_sa_photos_Next_question);
        Question_Name = findViewById(R.id.Learning_sa_photos_QuestionName);

        img2 = findViewById(R.id.Learning_sa_photos_img2);



        GoToTV = findViewById(R.id.Learning_sa_photos_goto);
        GotoQuestionNumber = findViewById(R.id.Learning_sa_photos_InputQuestionNumber);
        ErrorCheck = findViewById(R.id.Learning_sa_photos_ErrorCheck);
        Goto = findViewById(R.id.Learning_sa_photos_gotoButton);

        AnswerA = findViewById(R.id.Learning_sa_photos_Answer_A);
        AnswerB = findViewById(R.id.Learning_sa_photos_Answer_B);
        AnswerC = findViewById(R.id.Learning_sa_photos_Answer_C);
        AnswerD = findViewById(R.id.Learning_sa_photos_Answer_D);
        ...
        data = QuestionDatabase.getInstance(this).sa_photo_question_query().getAllQuestion();
        SA_photo_question question1 = data.get(0);
        SetQuestionData(question1,position,this);
        //Set first data
        //action bar
        getSupportActionBar().setTitle("Học Sa Hình");
    ...
    public void SetQuestionData(SA_photo_question questions ,int position , Context context){
        Question_Name.setText("Câu "+(position+1)+": "+questions.getQuestionName());



        //traffic signs 1
        if(!questions.getQuestionImgID().equalsIgnoreCase("null")){
            img2.setVisibility(View.VISIBLE);
            img2.setImageResource(QuestionDatabase.getInstance(this).sa_photos_query().getImgLink(questions.getQuestionID()));
        }else
        {
